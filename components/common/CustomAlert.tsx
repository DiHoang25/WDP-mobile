...
export default function CustomAlert({
    ...
    const config = {
        success: { icon: "checkmark-circle", color: AppColors.success, bg: "#ECFDF5" },
        error: { icon: "close-circle", color: AppColors.error, bg: "#FEF2F2" },
        warning: { icon: "warning", color: "#F59E0B", bg: "#FFFBEB" },
        info: { icon: "information-circle", color: AppColors.primary, bg: "#EFF6FF" },
    }[type];
                        ...
                        {currentButtons.map((btn, index) => (
                            <TouchableOpacity
                                ...
                                style={[
                                    ...
                                    currentButtons.length > 2 ? { width: '100%', marginBottom: 8 } : { flex: 1, marginHorizontal: 4 }
                                ]}
                                onPress={() => {
